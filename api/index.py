...
try:
    ...
    app = create_app()
    # Safety net for serverless: if APP_ENV is ever left unset (defaulting to
    # development/debug), Flask would otherwise re-raise request-time errors
    # raw instead of returning a handled 500 response, which Vercel reports
    # as an opaque "FUNCTION_INVOCATION_FAILED" crash. Forcing this off means
    # a real error always gets our own error page (see app/security.py) and
    # a logged traceback, in every environment.
